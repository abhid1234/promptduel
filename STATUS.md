...
- ⏳ **Phase 1: Dual-model load + streaming proof** — Vite + Transformers.js scaffold; load Gemma 4 270M and Qwen 2.5 0.5B in two parallel WebGPU contexts; basic side-by-side streaming UI with one shared prompt fanned out as two adversarial system prompts. Acceptance: type a topic, see both models stream simultaneously, no crashes on a phone. ~1 weekend.
- ⏸️ **Phase 2: Debate orchestration + voting + permalink** — 3-round logic (opening → rebuttal → closing). Vote bar. Cloudflare Worker + KV for permalink storage. "Share this duel" button. Acceptance: complete a full duel, vote, share the URL, friend opens the URL on a different device and sees the same duel re-run. ~half weekend.
